...
traversing the heap is typically parallelisable, and thus one
avenue for speeding up the garbage collector when multiple cores are
available is to perform parallel garbage collection.  GHC implements
such a scheme~\cite{Marlow:2008:PGG:1375634.1375637}, and \emph{parallel collection} is the primary source of
complexity in top-level loop of the garbage collector.

Note the distinction between parallel collection and \emph{concurrent
  collection}; in the latter, the GC runs concurrently with user code
which is mutating values on the heap.  Concurrent collection requires
synchronization between the GC and the mutator, and consequently is
more complex.  GHC does not currently have a concurrent collector,
although we have experimented with a form of concurrent collection in
which individual cores have local heaps that can be collected
independently of activity on the other cores~\cite{local-heaps}.
